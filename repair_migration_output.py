...
PROTECTED_REASONS = {
    "missing_services_description",
    "unmapped_services_description",
    "multi_category_resource_type",
    "possible_duplicate",
}
...
def repair(input_path: Path, report_path: Path) -> dict:
    ...
    for record in records:
        ...
        protected = bool(PROTECTED_REASONS.intersection(original_reasons))
        website_repaired = (
            not protected
            and
        ...
        if not protected and WEBSITE_REASON in reasons and plausible_bare_url(raw_website):
